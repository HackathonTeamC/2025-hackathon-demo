"""
Schedule Creator Lambda Function

スケジュール作成を処理するLambda関数。
ユーザーとの対話を通じて日程を調整し、Googleカレンダーにイベントを作成します。
"""
...
def lambda_handler(event, context):
    ...
    スケジュール作成のための対話処理を行います。
    Slackのmessageイベントまたは手動トリガーで呼び出されます。
    ...
    try:
        ...
        if body.get('type') == 'event_callback':
            ...
            if event_type == 'message':
                ...
                if not thread_ts:
                    return {'statusCode': 200, 'body': json.dumps({'message': 'Not a thread message'})}
                ...
                if event_data.get('bot_id'):
                    return {'statusCode': 200, 'body': json.dumps({'message': 'Bot message ignored'})}
